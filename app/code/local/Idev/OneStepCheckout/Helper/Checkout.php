...
class Idev_OneStepCheckout_Helper_Checkout extends Mage_Core_Helper_Abstract
{
    ...
    // this applies for the message "Email address already registered. Please login now or use a different email address."
    function isValidateEmail()
    {
        ...
        // if "Register on account without password" then no email validation should be performed
        if ($allow_without_password)
            return false;

        if($mode == 'auto_generate_account')
            return true;
    ...
    function hidePasswords()
    {
        ...
        if($this->settings['registration_mode'] == 'allow_guest')    {
            $getCreateAccount = Mage::app()->getRequest()->getPost('create_account', false);
    ...
    public function loadConfig()
    {
        ...
        if(is_object($persistentHelper)){
            $isPersistent = $persistentHelper->isPersistent();
        }
    ...
    public function checkEntryDev($domain, $serial)
    {
    ...
    /**
     * For checking if Magento admin config values are set to Yes or not
     * */
    private function isConfYes($flag)
    {
        if ($flag=="1") {
            return true;
        }
    ...
    /**
     * Magento admin switches for CollectTotals
     * */
    public function isYesSwitch_CollectTotal_CheckoutPhp_SaveOrder1()
    {
        return $this->isConfYes(Mage::getStoreConfig('onestepcheckout/collecttotals_switch/checkout_php_saveorder_1'));
    }

    public function isYesSwitch_CollectTotal_CheckoutPhp_SaveOrder2()
    {
        return $this->isConfYes(Mage::getStoreConfig('onestepcheckout/collecttotals_switch/checkout_php_saveorder_2'));
    }

    public function isYesSwitch_CollectTotal_AjaxcontrollerPhp_AddCouponAction1()
    {
        return $this->isConfYes(Mage::getStoreConfig('onestepcheckout/collecttotals_switch/ajaxcontroller_php_addcouponaction_1'));
    }

    public function isYesSwitch_CollectTotal_AjaxcontrollerPhp_SaveBillingAction1()
    {
        return $this->isConfYes(Mage::getStoreConfig('onestepcheckout/collecttotals_switch/ajaxcontroller_php_savebillingaction_1'));
    }

    public function isYesSwitch_CollectTotal_AjaxcontrollerPhp_SetMethodsSeparateAction_SetFlag_1()
    {
        return $this->isConfYes(Mage::getStoreConfig('onestepcheckout/collecttotals_switch/ajaxcontroller_php_setmethodsseparateaction_setflag_1'));
    }

    // EOF magento admin switches
