...
class Magestore_Madapter_Helper_Data extends Mage_Core_Helper_Abstract {
    ...
    public function encodeOrderJson($label, $data, $tax, $subTotal,$grandTotal,$discount, $coupon, $message) {
        $json = null;
        try {
            if (count($data))
                $json = Mage::helper('core')->jsonEncode(array($label => $data, 'tax' => $tax,'sub_total'=> $subTotal,'grand_total' => $grandTotal,'discount'=>$discount,'coupon_code' => $coupon, 'status' => 'SUCCESS'));
            else
    ...
    public function convertDataBilling($data) {
        ...
        $billing = array();
        $billing['address_id'] = isset($data['b_address_id']) == true ? $data['b_address_id'] : '';
        ...
        $billing['customer_address_id'] = $customerAddressId;
    ...
    public function convertDataShipping($data) {
        ...
        $shipping = array();
        $shipping['address_id'] = isset($data['s_address_id']) == true ? $data['s_address_id'] : '';
        ...
        $shipping['customer_address_id'] = $customerAddressId;
        return $shipping;
    }
