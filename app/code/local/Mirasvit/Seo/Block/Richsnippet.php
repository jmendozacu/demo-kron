...
class Mirasvit_Seo_Block_Richsnippet extends Mage_Core_Block_Template
{
    ...
    public function categorySnippetsFilter() {
        ...
        if ($this->getConfig()->getCategoryRichSnippets(Mage::app()->getStore()->getId()) == Mirasvit_Seo_Model_Config::CATEGYRY_RICH_SNIPPETS_CATEGORY) {
            ...
            $minPriceProductCollection = clone $productRatingCollection;
            ...
            $productRatingCollection->addFieldToFilter('rating_summary', array("nin" => array(0, NULL)));
            ...
            if ($productRatingCollection->getSize() > 0) {
                ...
                if ($this->getConfig()->getRichSnippetsRewiewCount(Mage::app()->getStore()->getStoreId()) == Mirasvit_Seo_Model_Config::REVIEWS_NUMBER) {
                    ...
                    $this->categorySnippetsRating      = $summaryRating/$productRatingCollection->getSize();
                } else { //Mirasvit_Seo_Model_Config::PRODUCTS_WITH_REVIEWS_NUMBER
                    $this->categorySnippetsRatingCount = $productRatingCollection->getSize();
                    $this->categorySnippetsRating      = $summaryRating/$this->categorySnippetsRatingCount;
                }
            }
        }
    ...
    public function isHide() {
        return $this->getConfig()->isHideCategoryRichSnippets(Mage::app()->getStore()->getId());
    }
}
