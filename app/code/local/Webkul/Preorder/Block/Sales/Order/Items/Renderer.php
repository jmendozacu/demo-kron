 ...
 */
class Webkul_Preorder_Block_Sales_Order_Items_Renderer extends Mage_Sales_Block_Order_Item_Renderer_Default
{
    ...
    public function getValueHtml($item)
    {
        if ($attributes = $this->getSelectionAttributes($item)) {
            ...
            $percent_accept=Mage::getStoreConfig('preorder/preorder_options/payment_for_preorder');
            $_product = Mage::getModel('catalog/product')->loadByAttribute('name',$item->getName());
            $attr = $_product->getResource()->getAttribute('wk_preorder');
            $val_id = $attr->getSource()->getOptionId('Enable');
            if($_product->getdata('wk_preorder')==$val_id && Mage::getModel('cataloginventory/stock_item')->loadByProduct($_product->getId())->getIsInStock()!=1) {
              $price=($price*$percent_accept)/100; 
            }
            return sprintf('%d', $attributes['qty']) . ' x ' .
                $this->escapeHtml($item->getName()) .
                " " . $this->getOrder()->formatPrice($price);
        } else {
