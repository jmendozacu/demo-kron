...
class Magestore_SimiPOS_Helper_Magestore extends Mage_Core_Helper_Abstract
{
    ...
    {
    	$storeUrl = Mage::app()->getDefaultStoreView()->getBaseUrl(
            Mage_Core_Model_Store::URL_TYPE_LINK,
            Mage::getStoreConfigFlag('web/secure/use_in_adminhtml')
        );
        if (Mage::getStoreConfig('web/url/use_store')) {
        	if ($pos = strpos($storeUrl, '/index.php')) {
        		$storeUrl = substr($storeUrl, 0, $pos) . substr($storeUrl, $pos+10);
        	}
        }
        return $storeUrl;
    }
	...
	public function login($username, $password)
	{
		...
		foreach ($result->getTerms() as $id => $data) {
			...
			if ($data['type'] == 2) {
				// Trial
				if (empty($termLabel)) {
					$termLabel = $this->__('Free for 1 device');
					if ($data['status'] == 2) {
	...
	{
		if (!Mage::getStoreConfig('simipos/account/username')) {
			return ;
		}
		
		if ($user->getData('email') == Mage::getStoreConfig('simipos/account/username')) {
			if ($user->getOrigData('password') != $user->getData('password')) {
				try {
				    $this->_magestore()->updateCustomer($user->getEmail(), $user->getPassword());
				} catch (Exception $e) {}
			}
			return ;
		}
