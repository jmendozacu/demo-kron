...
var OneStepCheckoutLoginPopup = Class.create(
    ...
    initialize: function(options) {
        ...
        this.keypress_handler = function(e) {
            if(e.keyCode == Event.KEY_RETURN) {
                ...
                if(this.mode == 'login') {
                    this.login_handler(e);
                } else if(this.mode == 'forgot') {
                    this.forgot_password_handler(e);
                }
            }
        }.bind(this);
    ...
    bindEventHandlers: function() {
        ...
        if(this.forgot_password_link){
            this.forgot_password_link.observe(
                'click', function(e) {
                e.preventDefault();
        ...
        if(this.forgot_password_button){
            this.forgot_password_button.observe(
                'click',
       ...
       if(this.popup){
           this.popup_container.observe('keypress', this.keypress_handler);
...
function oscUpdateCart(element, url, separate_url){
    ...
    var incrementElem = element.up('tr').select('input').first().next();
    if(qtyElem){
    ...
    var incrVal = 1.0;
    if(incrementElem){
        if (incrementElem.value!="") incrVal = parseFloat(incrementElem.value);
    }
    if(element.name == 'add'){
        if(qty >= 0){
            qtyElem.value = qty + incrVal;
        } else if(!qty){
            qtyElem.value = 1;
        }
            ...
            if(transport.status == 200){
                ...
                if(response.redirect){
                    ...
                    if(shipping_methods){
                        ...
                        $$('dl.shipment-methods input').invoke(
                            'observe', 'click', function() {
                            $$('div.onestepcheckout-shipment-method-error').each(
                                function(item) {
                    ...
                    if(payment_methods){
                        ...
                        $$('div.payment-methods input[name^=payment\[method\]]').invoke(
                            'observe', 'click', function() {
                            $$('div.onestepcheckout-payment-method-error').each(
                                function(item) {
                                new Effect.Fade(item);
                                }
                            );
    ...
    new Ajax.Request(
        ...
        onSuccess: function(transport){
            if(transport.status == 200)    {
                    ...
                    $$('div.payment-methods input[name="payment\[method\]"]', 'div.payment-methods input[name="payment\[useProfile\]"]').invoke(
                        'observe', 'click', function() {
                        $$('div.onestepcheckout-payment-method-error').each(
                            function(item) {
                            new Effect.Fade(item);
...
Billing.prototype = {
    ...
    newAddress: function(isNew){
        ...
        $$('input[name^=billing]', 'select[id=billing:region_id]').each(
            function(e){
...
var Shipping = Class.create();
    ...
    newAddress: function(isNew){
        ...
        $$('#shipping_address input[name^=shipping],#shipping_address select[id=shipping:region_id]').each(
            function(e){
...
var Payment = Class.create();
    ...
    var gaSettings = Class.create(
        {

        isLibraryGa: function() {
        ...
        isAnyGALibraryFound: function() {
            if (this.isLibraryGa()||this.isLibraryAnalytics()) {
                return true;
            } else {
                return false;
            }
    ...
    var oscAdvancedGoogle = Class.create(
        ...
        initialize: function(Config) {
            ...
            if (gas.isAnyGALibraryFound()) {
                if ( gas.isLibraryGa()) {
                    //console.log("Idev OSC: ga.js found");
                } else if ( gas.isLibraryAnalytics() ) {
                    //console.log("Idev OSC : analytics.js found");
                }
            }
            else {
                //console.error("Idev OSC: Google Analytics is not found");
                return;
            }
            ...
            trackEvent: function(msg) {
                ...
                if (gas.isAnyGALibraryFound()) {
                    ...
                    if (!this.hasTracked(push_args)) {
                        ...
                        // old way
                        if (gas.isLibraryGa()) {
                            window._gaq.push(push_args);
                        }
                        // new analytics.js way
                        else if (gas.isLibraryAnalytics()) {
                            window.ga(
                                'send',
                                'event',
