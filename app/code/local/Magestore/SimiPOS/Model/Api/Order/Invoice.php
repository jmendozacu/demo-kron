...
class Magestore_SimiPOS_Model_Api_Order_Invoice extends Magestore_SimiPOS_Model_Api_Abstract
{
     ...
     */
    public function apiCancel($invoiceIncrementId)
    {
    	$invoice = Mage::getModel('sales/order_invoice')->loadByIncrementId($invoiceIncrementId);
        if (!$invoice->getId()) {
            throw new Exception($this->_helper->__('Invoice is not found.'), 96);
        ...
        if (!$invoice->canCancel()) {
        	throw new Exception($this->_helper->__('Invoice cannot be canceled.'), 98);
        }
        try {
        	$invoice->cancel();
        	$invoice->getOrder()->setIsInProcess(true);
     ...
     * @param $invoiceIncrementId
     */
    public function apiVoid($invoiceIncrementId)
    {
        $invoice = Mage::getModel('sales/order_invoice')->loadByIncrementId($invoiceIncrementId);
        if (!$invoice->getId()) {
            throw new Exception($this->_helper->__('Invoice is not found.'), 96);
        ...
        if (!$invoice->canVoid()) {
        	throw new Exception($this->_helper->__('Invoice cannot be voided.'), 99);
        }
        try {
            $invoice->void();
            $invoice->getOrder()->setIsInProcess(true);
