 ...
 */
class Magestore_SimiPOS_Model_Total_Invoice_Cash extends Mage_Sales_Model_Order_Invoice_Total_Abstract
{
    public function collect(Mage_Sales_Model_Order_Invoice $invoice)
    {
        ...
        if ($invoice->isLast()) {
            $cash = $order->getSimiposCash();
            $baseCash = $order->getSimiposBaseCash();
            foreach ($order->getInvoiceCollection() as $existedInvoice) {
                if ($cash > 0.0001) {
                    $cash -= $existedInvoice->getSimiposCash();
                    $baseCash -= $existedInvoice->getSimiposBaseCash();
                }
            }
        } else {
            ...
            foreach ($order->getInvoiceCollection() as $existedInvoice) {
                if ($maxcash > 0.0001) {
                    $maxcash -= $existedInvoice->getSimiposCash();
                    $maxbaseCash -= $existedInvoice->getSimiposBaseCash();
                }
            }
            if ($cash > $maxcash) {
                $cash = $maxcash;
