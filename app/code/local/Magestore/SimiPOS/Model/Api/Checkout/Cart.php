    ...
    extends Magestore_SimiPOS_Model_Api_Checkout_Abstract
{
    ...
    public function apiInfo()
    {
        ...
        $result['currency_code'] = $quote->getData('quote_currency_code');
    ...
    protected function _totals($quote)
    {
        ...
        foreach ($totals as $total) {
        	if (abs($total->getValue()) < 0.0001
        	   || in_array($total->getCode(), array('reward_label'))
        	) {
        		continue;
        	}
            $result[$index++] = array(
     ...
     * @return array
     */
    public function apiAddress($type = 'billing')
    {
        $quote = $this->_getQuote();
        if ($type == 'billing') {
    ...
    public function apiCreateOrder($configData = array())
    {
        ...
        try {
            ...
            $quote->setLocationId($this->getUser()->getLocationId());
        ...
        if (($order->getSimiposCash() >= $order->getGrandTotal()
        	...
        	if ($order->getSimiposCash() >= $order->getGrandTotal()) {
        		if ($order->getPayment()->getMethodInstance()->getCode() == 'cashin') {
        			// create invoice with full paid by cash
        			$order->setSimiposCash(0)
        			    ->setSimiposBaseCash(0);
        		}
        	}
