 ...
 * 
 */
class Glace_Fullpagecache_Monitor
{
    const TMP_PATH = 'var/glace_pagecache';
    ...
    protected $_excludedBlockPages = array(
        ...
        'persistent',
        '/checkout/',
        'fullpagecache/unavailable/return',
    );
    ...
    {
        $this->_isEnabled = (bool)class_exists('Glace_Aitloadmon_Collect',false);
        if($this->_isEnabled) {
            $this->_loadLevel = (int)Glace_Aitloadmon_Collect::getLoadLevel();
        }
        setcookie('glaceloadmon_loadlevel', $this->_loadLevel);
    ...
    {
        if(strpos($this->_getRequestUri(),'fullpagecache/unavailable/check'))
        {
            $this->_blockGetLoad();
        }
        if(strpos($this->_getRequestUri(),'fullpagecache/unavailable/mail') && isset($_POST['email']))
        {
    ...
    {
        $excludedBlockPages = array_merge($this->_excludedBlockPages, $this->_config[Glace_Fullpagecache_Mainpage::ADMIN_PATH]);
    ...
    protected function _blockCustomers()
    {
        ...
        if(strpos($requestUri,'fullpagecache/unavailable'))
        {
        ...
        else
        {
            ...
            $this->_setRequestUri($_SERVER['SCRIPT_NAME'].'/fullpagecache/unavailable/');
        }
    }
