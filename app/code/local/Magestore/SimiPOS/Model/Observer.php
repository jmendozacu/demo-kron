...
class Magestore_SimiPOS_Model_Observer
{
    ...
    public function orderPaymentMethodActive($observer)
    {
        $session = Mage::getSingleton('simipos/session');
        if (!$session->getSessionId()) {
            return ;
        }
        $methodString = Mage::getStoreConfig('simipos/checkout/payment_method', $session->getCurrentStoreId());
        if (!$methodString) {
        	return ;
        }
        $forceMethods = explode(',', $methodString);
        $method = $observer['method_instance'];
        if (in_array($method->getCode(), $forceMethods)) {
