 ...
 * 
 */ 
class Glace_Extendedordergrid_Model_Order_Item extends Mage_Core_Model_Abstract
{
    ...
    public function getMappedColumns(){
        $predefinedColumns = array(
            'extendedordergrid_item_id', 'item_id'
        );
        
        $tableName = Mage::getSingleton('core/resource')
                ->getTableName('ciextendedordergrid/order_item');
    ...
    public function clearTemporaryData(){
        ...
        $orderItemProductTableName = Mage::getSingleton('core/resource')
            ->getTableName('ciextendedordergrid/order_item_product');
    ...
    protected function initProduct2OrderItemRelation($ordersIds = array(), $fullRemap = FALSE, $orderItem = NULL){
        ...
        if ($orderItem !== NULL){
            $insertQuery = '
                ...
                ('.$orderItem->getItemId().', ' . $orderItem->getProductId() . ', ' . $orderItem->getStoreId() . ' )
            ';
    ...
    protected function getMappingData($mappingColumns){
        ...
        $mappingData = $collection->getItems();
...
//        }
    ...
    function getUnmappedOrders(){
        ...
        $collection->getSelect()->join(
            array(
                'cmsideas_order_item_product' => $collection->getTable('ciextendedordergrid/order_item_product')
            ),
            'main_table.item_id = cmsideas_order_item_product.item_id', 
            array()
        );
        ...
        $collection->getSelect()->joinLeft(
            ...
            'product.entity_id = cmsideas_order_item_product.product_id', 
            array()
        );
