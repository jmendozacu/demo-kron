...
class Magestore_Shopbybrand_Block_Adminhtml_Report_Statistic_Grid extends Mage_Adminhtml_Block_Widget_Grid {
        ...
        $this->addColumn('action', array(
            ...
            'actions' => array(
                array(
                    ...
                    'url' => array('base' => '*/shopbybrand_brand/edit/store/' . $this->getRequest()->getParam('store') . '/reportSales/1/from/' . str_replace("/", "_", $this->getFilter('report_from')) . '/to/' . str_replace("/", "_", $this->getFilter('report_to')) . '/period/' . $period),
                    'field' => 'id'
            )),
            'filter' => false,
            'sortable' => false,
    ...
    public function getRowUrl($row) {
        ...
        if ($this->getFilter('report_from'))
            $period = 'custom';
        return $this->getUrl('*/shopbybrand_brand/edit', array(
                    'id' => $row->getId(),
                    'store' => $store,
                    'reportSales' => 1,
                    'period' => $period,
