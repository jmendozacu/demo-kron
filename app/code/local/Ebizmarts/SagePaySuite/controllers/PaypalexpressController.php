...
class Ebizmarts_SagePaySuite_PaypalexpressController extends Mage_Core_Controller_Front_Action
{
    ...
    private function _initCheckout()
    {
        ...
        else if(Mage::getStoreConfig('payment/sagepaypaypal/shipping_method_selection',Mage::app()->getStore()) == 'estimate' && !$quote->getShippingAddress()->getShippingMethod()){
	...
	public function callbackAction()
	{
        ...
        if(Mage::getStoreConfig('payment/sagepaypaypal/shipping_method_selection') == 'review'){
            //second review step
            $this->_redirect('sgps/paypalexpress/review', array('_secure'=>true));
        }else{
            //avoid second review step
            //$this->_checkout->supressReviewStep();
            $this->_checkout->prepareOrderReview();
            $this->_redirect('sgps/paypalexpress/placeOrder', array('_secure'=>true));
        }
