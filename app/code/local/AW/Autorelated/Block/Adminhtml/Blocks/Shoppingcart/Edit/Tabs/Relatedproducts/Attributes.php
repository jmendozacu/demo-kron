    ...
    public function getProductAttributes()
    {
        if ($this->_productAttributes === null) {
            $this->_productAttributes = Mage::getModel('awautorelated/source_catalog_product_attributes')
                ->toShortOptionArray()
            ;
            $_unsetAttributeCodes = array(
                'attribute_set_id',
                'category_ids'
            );
            foreach ($_unsetAttributeCodes as $_attributeCode) {
                if (isset($this->_productAttributes[$_attributeCode])) {
                    unset($this->_productAttributes[$_attributeCode]);
                }
            }
        }
